...
class PostOffice {
  public:
    ...
    void ActivateAutoAck(); // Used by TCP PostOffice to ack the message when arrived

    void NotifyOurself(int id);
            //Send a message to ourself without using network
  ...
  private:
    ...
    void SendAck(Mail *mail); // To send ack
};
