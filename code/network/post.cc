...
{ 
    DEBUG('w', "Waiting for mail in mailbox\n");
...
PostOffice::PostalDelivery()
{
    ...
    if (autoAck && mailHdr.to != 0){ // We ack only messages and not ack or timersignals
        mail = new Mail(pktHdr,mailHdr, buffer + sizeof(MailHeader));
        SendAck(mail);
        delete mail;
    }
...
// Send a ack if the message received
void
PostOffice::SendAck(Mail *mail){
    char* buffer = new char[MaxPacketSize];
    const char *ack = "It is an ACK";
    PacketHeader outPktHdr;
    MailHeader outMailHdr;
    TCPHeader outtcpHeader;
    outPktHdr.to = mail->pktHdr.from;
    outMailHdr.to = 0;
    outtcpHeader.type = TYPE::ACK;
    outtcpHeader.length = strlen(ack) + 1;
    outMailHdr.length = outtcpHeader.length + sizeof(TCPHeader);

    bcopy(&outtcpHeader, buffer, sizeof(TCPHeader));
    bcopy( ack, buffer + sizeof(outtcpHeader), outtcpHeader.length);

    Send(outPktHdr, outMailHdr, buffer);
    delete buffer;
}
...
//----------------------------------------------------------------------
// PostOffice::NotifyOurself
//  Send a message of type TIMERSIGNAL to our box 0 without passing by network
//
//	"id" -- the id of the timersignal
//----------------------------------------------------------------------

void
PostOffice::NotifyOurself(int id)
{
    const char* data = "It is a TimerSignal";
    char* buffer = new char[MaxPacketSize];	// space to hold concatenated
						// mailHdr + data

    TCPHeader tcpHeader;
    tcpHeader.id = id;
    tcpHeader.type = TYPE::TIMERSIGNAL;
    tcpHeader.length = strlen(data) + 1;
    MailHeader mailHdr;
    mailHdr.to = 0;
    mailHdr.from = 0;
    mailHdr.length = tcpHeader.length + sizeof(TCPHeader);
    PacketHeader pktHdr;
    pktHdr.to = netAddr;
    pktHdr.from = netAddr;
    pktHdr.length = mailHdr.length + sizeof(MailHeader);
    ...
    bcopy(&tcpHeader, buffer, sizeof(TCPHeader));
    bcopy( data, buffer + sizeof(tcpHeader), tcpHeader.length);

    // send of auto mail
    boxes[0].Put(pktHdr, mailHdr, buffer);
    fflush(stdout);
    delete [] buffer;			// we've sent the message, so
					// we can delete our buffer
}
