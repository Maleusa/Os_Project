...
Thread::~Thread ()
{
    ...
    this->space->DeAllocateStack(stackID); // we clear the bitmap to signifie that our stack can be used
...
Thread::Fork (VoidFunctionPtr func, int arg)
{
    ...
    // TODO
    if (arg != -1) this->space = currentThread->space;
...
int Thread::waitingOn(int tidtowait,int retvaluaddr){
    return space->do_Wait(tidtowait,retvaluaddr);
}
// End of addition
...
void Thread::DeleteStack(){
    space->DeAllocateStack(stackID);
}


void Thread::setPid(){
    this->pid=this->space->GetPid();
}
